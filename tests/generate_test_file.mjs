...
import { ASSERT, INPUT_HEADER, OUTPUT_QUINTICK, OUTPUT_QUINTICKJS } from './utils.mjs';
...
export function generateTestFile(tob) {
  ...
  let relfile = file.slice(file.indexOf('escaya'));

  let descPath = path
    .dirname(relfile.slice(relfile.indexOf('tests/testcases/') + 10))
    .split('/')
    .map((s) => s.replace(/_/g, ' ').replace(/x([a-z\d]{4})/g, (_, s) => String.fromCharCode(parseInt(s, 16))))
    .join(' : ');
  let descFile = path
    .basename(relfile)
    .slice(0, -3)
  ...
  let newData = (tob.oldData = `# escaya parser test case
