...
// Based on similar script in tenko repo
// https://github.com/pvdz/tenko/tree/master/tests
...
import { generateTestFile } from './generate_test_file.mjs';
...
let ASSERT = (...args) => {
  if (NO_FATALS)
    try {
      ...
      console.error('Assertion error (squashed by NO_FATALS):', e.stack);
    }
  else _ASSERT(...args);
};

console.log('Start of escaya test suite');

// TODO: remove unused options
const INPUT_OVERRIDE = decodeUnicode(
  process.argv.includes('-F')
    ? fs.readFileSync(process.argv[process.argv.indexOf('-F') + 1], 'utf8')
...
let [a, b, c] = [
  process.argv.includes('--sloppy'),
  process.argv.includes('--strict'),
  process.argv.includes('--module')
];
...
const RUN_VERBOSE_IN_SERIAL =
  process.argv.includes('--serial') ||
  (!SEARCH && !TESTS_ONLY && (INPUT_OVERRIDE || TARGET_FILE || STOP_AFTER_TEST_FAIL || STOP_AFTER_FILE_FAIL));
...
// TODO: make it work with ts
const ESCAYA_DEV_FILE = './parse.mjs';
const ESCAYA_PROD_FILE = './parse.mjs';
...
if (process.argv.includes('-?') || process.argv.includes('--help')) {
  ...
  Options:
    -b  --build   Use prod build instead of dev source for escaya in this call (assumes built in \`/build/...\`; \`./t z\`)
...
const FORCED_ES_TARGET = TARGET_ES6
  ? 6
  : TARGET_ES7
  ? 7
  : TARGET_ES8
...
if ((REDUCING || REDUCING_PRINTER) && !TARGET_FILE && !INPUT_OVERRIDE)
  throw new Error('Can only use `--min` and `--min-parser` together with `-f` or `-i`');
if (NO_FATALS) console.log(BLINK + 'NO_FATALS enabled. Do not blindly commit result!!' + RESET);
if (USE_BUILD) console.log('Using PROD build of escaya');
...
function coreTest(
  ...
  code = tob.inputCode,
  verbose = !!(INPUT_OVERRIDE || TARGET_FILE)
) {
  wasHits = [];
  ...
  try {
    if (verbose) {
      console.time('Pure escaya parse time');
    ...
    if (verbose) {
      console.timeEnd('Pure escaya parse time');
      if (CONCISE) return;
    }
    if (tob.shouldFail) {
    ...
    if (!NO_FATALS && AUTO_UPDATE && tob.continuePrint && !CONFIRMED_UPDATE && !INPUT_OVERRIDE && !TARGET_FILE) {
      console.error(
        BOLD +
          ...
          tob.file +
          RESET +
          ' was explicitly marked to pass, but it failed somehow;\n' +
          RED +
          tob.continuePrint +
          RESET
      );
  ...
  if (TEST_BABEL && (!Number.isFinite(tob.inputOptions.es) || TARGET_FILE || INPUT_OVERRIDE)) {
    [babelOk, babelFail, zasb] = compareBabel(
      code,
      !e,
      testVariant,
      ENABLE_ANNEXB || annexB,
  ...
  if (TEST_ACORN) {
    if (tob.fileShort.startsWith('tests/testcases/regexes/range_surrogate_head_end/')) {
      ...
      [acornOk, acornFail, zasa] = compareAcorn(
        code,
        !e,
        testVariant,
        ENABLE_ANNEXB || annexB,
...
async function postProcessResult(tob /*: Tob */, testVariant /*: "sloppy" | "strict" | "module" */, annexB) {
  let {
    parserRawOutput: {
      [testVariant + annexB]: { r, e, stdout, babelOk, babelFail, zasb, nodeFail, acornOk, acornFail, zasa }
    },
    file
  } = tob;
  ...
  if (TEST_BABEL && annexB && testVariant !== 'strict') {
    ...
    if (Number.isFinite(tob.inputOptions.es)) {
      ...
      ASSERT(
        !babelOk !== !babelFail,
        'babel should have run, should either pass or fail, not both, not neither [file = ' + file + ' ]'
      );
  ...
  let nodeOutput =
    nodeFail === undefined
      ...
      ? !nodeFail && e
        ? 'Node compiled a function with a strictmode header and this input without error\n\n'
        : nodeFail && !e
        ? 'Node threw an error while compiling a function with a strictmode header and this input\n\n'
        : ''
      : !nodeFail && e
      ? 'Node compiled a function with this input without error\n\n'
      : nodeFail && !e
      ? 'Node threw an error while compiling a function with this input:\n' + nodeFail + '\n\n'
      : '';
...
async function runTest(list, parse, testVariant /*: "sloppy" | "strict" | "module" */, annexB) {
  if (!RUN_VERBOSE_IN_SERIAL)
    console.log(' - Now testing', INPUT_OVERRIDE ? 'for:' : 'all cases for:', testVariant, 'annexb=', annexB);
      ...
      if (SEARCH) {
        ...
        if (INPUT_OVERRIDE) {
          ...
          if (hitsToReport.length) {
            ...
            let root =
              `${box} Input ${BOLD + 'WAS' + RESET} hit` +
              (hitsToReport.length > 1 ? ` (${hitsToReport.length}x)` : '') +
              '    ';
            hitsToReport.forEach((hit, i) => {
              if (i === 0) PRINT_HIT(root + '(' + hit + ')');
              else
                PRINT_HIT(
                  ' ' + '('.padStart(root.length - (box.length - 3 + BOLD.length + RESET.length), ' ') + hit + ')'
                );
            });
          } else {
            PRINT_HIT(
              `[${(e && e.message.includes('TODO') ? 'T' : e ? RED + 'x' : GREEN + 'v') + RESET}] Input ${
                'was ' + BOLD + 'NOT' + RESET
              } hit`
            );
          }
  ...
  if (!RUN_VERBOSE_IN_SERIAL)
    console.log('   Have', list.length, 'results, totaling', bytes, 'bytes, ok = ', ok, ', fail =', fail);
...
function showDiff(tob) {
  ...
  let newData = TEST_ACORN || TEST_BABEL ? tob.newData : encodeUnicode(tob.newData);
...
async function runTests(list, parse) {
  if (!RUN_VERBOSE_IN_SERIAL) console.time('$$ Total runtime');
  if (!RUN_VERBOSE_IN_SERIAL)
    console.log(
      'Now actually running all',
      list.length,
      'test cases... 4x! Single threaded! This may take some time (~20s on my machine)'
    );
  if (RUN_SLOPPY) await runTest(list, parse, TEST_SLOPPY, false);
  if (RUN_STRICT) await runTest(list, parse, TEST_STRICT, false);
  if (RUN_MODULE) await runTest(list, parse, TEST_MODULE, false);
  if (RUN_SLOPPY) await runTest(list, parse, TEST_SLOPPY, true);
  if (RUN_MODULE) await runTest(list, parse, TEST_MODULE, true);
      ...
      if (newOutput !== oldOutput) {
        ...
        console.log(
          BOLD +
            ...
            tob.file +
            '"' +
            (TEST_BABEL ? ' --test-babel' : '') +
            (TEST_ACORN ? ' --test-acorn' : '') +
            '\n'
        );
    ...
    for (let i = 0; i < list.length; ++i) {
      ...
      if (newData !== oldData || FORCE_WRITE) {
        ...
        if (tob.continuePrint) {
          ...
          if (!(await yn('File was not changed, invariant was broken and written anyways. Continue testing?')))
            process.exit();
        }
      }
    }
...
async function loadParserAsync() {
  let parse;
  ...
  ({
    ...
    WEB_COMPAT_OFF,
    toktypeToString
  } = await import(path.join(dirname, USE_BUILD ? ESCAYA_PROD_FILE : ESCAYA_DEV_FILE)));
  if (!RUN_VERBOSE_IN_SERIAL) console.timeEnd('$$ Parser load');

  return parse;
}
async function runAndRegenerateList(list, parse) {
  await runTests(list, parse);
  if (TESTS_ONLY) {
  ...
  if (!SEARCH && !CONCISE) {
    ...
    if (RUN_STRICT) {
      ...
      if (list[0].newOutputStrict !== false) {
        ...
        else if (RUN_SLOPPY && list[0].newOutputStrict === list[0].newOutputSloppyAnnexB)
          console.log('Same as sloppy with annexB');
      ...
      if (list[0].newOutputStrictAnnexB !== false) {
        ...
        else if (RUN_SLOPPY && list[0].newOutputStrictAnnexB === list[0].newOutputSloppyAnnexB)
          console.log('Same as sloppy with annexB');
        else if (RUN_SLOPPY && list[0].newOutputStrictAnnexB === list[0].newOutputStrict)
          console.log('Same as strict without annexB');
    ...
    if (RUN_MODULE) {
      ...
      if (list[0].newOutputModule !== false) {
        ...
        else if (RUN_SLOPPY && list[0].newOutputModule === list[0].newOutputSloppyAnnexB)
          console.log('Same as sloppy with annexB');
        else if (RUN_STRICT && list[0].newOutputModule === list[0].newOutputStrict) console.log('Same as strict');
        else if (RUN_STRICT && list[0].newOutputModule === list[0].newOutputStrictAnnexB)
          console.log('Same as strict with annexB');
      ...
      if (list[0].newOutputModuleAnnexB !== false) {
        ...
        else if (RUN_SLOPPY && list[0].newOutputModuleAnnexB === list[0].newOutputSloppyAnnexB)
          console.log('Same as sloppy with annexB');
        else if (RUN_STRICT && list[0].newOutputModuleAnnexB === list[0].newOutputStrict) console.log('Same as strict');
        else if (RUN_STRICT && list[0].newOutputModuleAnnexB === list[0].newOutputStrictAnnexB)
          console.log('Same as strict with annexB');
        else if (RUN_SLOPPY && list[0].newOutputModuleAnnexB === list[0].newOutputModule)
          console.log('Same as module without annexB');
...
async function main(parse) {
  ...
  if (RUN_VERBOSE_IN_SERIAL) {
    for (let i = 0; i < list.length && !stopAsap; ++i) {
      let tob = list[i];
      await runAndRegenerateList([tob], parse);
      ...
      if (tob.compareHadMatchFailure && !tob.compareWhiteListed) {
        ...
        console.log(
          BOLD +
            ...
            tob.fileShort +
            RESET +
            ' (file not whitelisted to fail but it failed anyways, investigate)'
        );
        ...
        console.log(
          BOLD +
            ...
            tob.fileShort +
            RESET +
            ' (file whitelisted to fail and it failed)'
        );
        ...
        console.log(
          BOLD +
            ...
            tob.fileShort +
            RESET +
            ' (file whitelisted to fail but did not fail, remove from list)'
        );
        ...
        console.log(
          BOLD +
            ...
            tob.fileShort +
            RESET +
            ' (file skipped because it targets a specific ES version and we dont care about those cases here)'
        );
    ...
    console.log(
      'Match status: whitelisted: ' +
        skippedOtherParserList.length +
        'x, unexpected misses: ' +
        unxepctedFails.length +
        'x, unexpected passes: ' +
        unexpectedPass.length +
        'x'
    );
    if (unxepctedFails.length || unexpectedPass.length)
      console.log('Use -q to stop immediately on an unexpected miss/match');
  } else {
    await runAndRegenerateList(list, parse);
  }
...
async function gen(parse) {
  ...
  for (let ti = 0; ti < list.length; ++ti) {
    ...
    console.log(
      'Wrote',
      wrote,
      'new test files' + (skipped ? ', skipped ' + skipped + ' existing files' : ''),
      'dir:',
      genDir.slice(path.join(dirname, '..').length + 1)
    );
  }
}
function createAutoGeneratedTestFileContents(tob /*: Tob */, caseDir, title, c, params, code) {
  return `# escaya parser autogenerated test case

- From: ${tob.fileShort}
- Path: ${caseDir.slice(caseDir.indexOf('escaya') + 'escaya/'.length)}
...
${Object.getOwnPropertyNames(params)
  ...
  .join('')}${OUTPUT_QUINTICKJS}${code.replace(/#/g, c)}${OUTPUT_QUINTICK}`;
}
...
> Actual comments go here
  ```
  */
  let topGrep = /^\s*(# escaya parser (?:autogenerated )?test case)\n\s*\n(- From:.*?\s*\n)*(?:- (?:Path|Added|Modified):.*?\s*\n)*(> :: .*\s*\n>\s*\n)?(> ::>? .*\s*\n)?/;

  ASSERT(
    /\s*^# escaya parser (autogenerated )?test case/.test(tob.aboveTheFold),
    'all test cases should include this title: ' + tob.file + ' did not'
  );
  let fold = tob.aboveTheFold.replace(
    topGrep,
    (_, title, from) =>
      title +
      `
...
> :: ${path
        .dirname(tob.fileShort)
        .split('/')
        .filter((s) => !['tests', 'testcases', ''].includes(s))
        .map((s) => s.replace(/_/g, ' '))
        .join(' : ')
        .replace(/ +/g, ' ')
        .trim()}
...
function generateOutputBlock(tob) {
  ...
  return (tob.newData =
    ...
    generateOutputSloppy(
      sloppy,
      false,
      '',
      tob.newOutputSloppyBabel,
      tob.newOutputSloppyNode,
      tob.newOutputSloppyAcorn
    ) +
    ...
    generateOutputSloppy(
      ...
      true,
      sloppy,
      tob.newOutputSloppyAnnexBBabel,
      tob.newOutputSloppyAnnexBNode,
      tob.newOutputSloppyAnnexBAcorn
    ) +
    generateOutputModule(
      ...
      strict,
      sloppy,
      tob.newOutputModuleAnnexBBabel,
      tob.newOutputModuleAnnexBAcorn
    ) +
...
function generateOutputSloppy(sloppyOutput, annexB, sloppy, babelOutput, nodeOutput, acornOutput) {
  return (
    (annexB ? OUTPUT_HEADER_SLOPPY_ANNEXB : OUTPUT_HEADER_SLOPPY) +
    '\n' +
    'Parsed with script goal' +
    (annexB ? ' with AnnexB rules enabled' : '') +
    ' and as if the code did not start with strict mode header.\n' +
...
function generateOutputStrict(strictOutput, annexB, sloppyOutput, babelOutput, nodeOutput) {
  return (
    ...
    'Parsed with script goal' +
    (annexB ? ' with AnnexB rules enabled' : '') +
    ' but as if it was starting with `"use strict"` at the top.\n' +
...
function generateOutputModule(
  ...
  return (
    (annexB ? OUTPUT_HEADER_MODULE_ANNEXB : OUTPUT_HEADER_MODULE) +
    '\n' +
...
async function loadParsers() {
  let parse = await loadParserAsync();
  return parse;
}
