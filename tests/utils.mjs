...
console.log = (...args) => {
  if (
    prefixLogOrigin &&
    (args.length > 1 || (args.length === 1 && (typeof args[0] !== 'string' || args[0].trim() !== '')))
  ) {
...
export class Tob {
  constructor(file, data) {
    ...
    this.file = file;
    this.fileShort = file.startsWith('tests/') ? file : file.slice(file.indexOf('escaya') + 'escaya/'.length);
    this.oldData = data;
    this.newData = data;
    ...
    this.compareWhiteListed = false; // Do we know why a test isn't matching escaya's output? In that case ignore the error.
    this.compareHadMatchFailure = false; // At least one test that ran had a mis-match with escaya
...
function parseTestFile(tob) {
  ...
  ASSERT(
    oldData.includes(INPUT_HEADER),
    'missing input header in ' + tob.file + ' (did you mean to use `./t F` instead of `./t f`?)'
  );
  ...
  if (oldData.includes(OUTPUT_HEADER, inputCodeOffset)) {
    ...
    if (outputSloppyAnnexbHeaderOffset > 0 && outputSloppyAnnexbOffset > 0) {
      ...
      ASSERT(
        oldData.includes(OUTPUT_QUINTICK, outputSloppyAnnexbOffset + OUTPUT_QUINTICK.length),
        'output should contain strict output quintick'
      );
      tob.oldOutputSloppyAnnexB = oldData.slice(
        outputSloppyAnnexbOffset,
        oldData.indexOf(OUTPUT_QUINTICK, outputSloppyAnnexbOffset)
      );
    } else {
    ...
    if (outputStrictHeaderOffset > 0 && outputStrictOffset >= 0) {
      ...
      ASSERT(
        oldData.includes(OUTPUT_QUINTICK, outputStrictOffset + OUTPUT_QUINTICK.length),
        'output should contain strict output quintick'
      );
    ...
    if (outputModuleAnnexbOffset >= 0) {
      ...
      tob.oldOutputModuleAnnexB = oldData.slice(
        outputModuleAnnexbOffset,
        oldData.indexOf(OUTPUT_QUINTICK, outputModuleAnnexbOffset)
      );
    } else {
  ...
  tob.inputOptions = tob.inputHead
    ...
    .reduce((obj, s) => {
      ...
      ASSERT(
        is === '=',
        'separate the key and value of input options with a `=` and surrounding spaces',
        ' ' + file + ' ',
        s
      );
  ...
  ASSERT(
    ...
    'options have hardcoded support in the test suite so if a new option needs support, make sure to connect it first, then update this assert. Unhandled options: ' +
      JSON.stringify(unsupported) +
      ', file: ' +
      tob.fileShort
  );
...
async function readFiles(files) {
  return await Promise.all(files.map(promiseToReadFile)).catch((e) => {
    throw new Error(e);
  });
}
